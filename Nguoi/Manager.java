...
public class Manager extends User implements InfoDisplayable {
     ...
     * user.displayInfo(); // Bạn có thể sử dụng phương thức này để hiển thị thông
     * tin người dùng
    ...
    public void viewPurchasedUsers(ArrayList<User> userList, List<Product> productList) {
        ...
        for (User user : userList) {
            ...
            List<Product> purchaseHistory = user.getPurchaseHistory(); // Lấy danh sách sản phẩm đã mua
            ...
            if (purchaseHistory.isEmpty()) {
                ...
                for (Product purchasedProduct : purchaseHistory) {
                    System.out.println(" - " + purchasedProduct.getProductName() +
                            " (Số lượng: " + purchasedProduct.getQuantity() + ")");

                    // Tìm kiếm sản phẩm trong danh sách sản phẩm để hiển thị chi tiết
                    for (Product product : productList) {
                        if (product.getProductName().equals(purchasedProduct.getProductName())) {
    ...
    public void performAction() {
        System.out.println(getUsername() + " đang quản lý người dùng."); // Hiển thị thông báo người quản lý đang thực
                                                                         // hiện hành động
    }
}
